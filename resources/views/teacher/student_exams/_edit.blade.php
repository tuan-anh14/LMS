...
<form method="post" action="{{ route('teacher.student_exams.update', ['student' => $student->id, 'lecture' => $lecture->id,]) }}" class="ajax-form">
    @csrf
    @method('put')
    ...
    <div class="form-group">
        ...
        <select name="attendance_status" id="attendance-status" class="form-control select2" required>
            ...
            @foreach (AttendanceStatusEnum::getConstants() as $attendanceStatus)
                <option value="{{ $attendanceStatus }}" {{ $attendanceStatus == $lecture->pivot->attendance_status ? 'selected' : '' }}>@lang('lectures.' . $attendanceStatus)</option>
            @endforeach
        </select>
    </div>
