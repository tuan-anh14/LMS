...
<form method="post" action="{{ route('teacher.student_lectures.update', $studentLecture->id) }}" class="ajax-form">
    @csrf
    @method('put')
        ...
        <div class="col-md-4">
            <div class="form-group">
                ...
                <select name="center_id" class="form-control select2" disabled>
                    <option value="">{{ $studentLecture->center->name }}</option>
                </select>
            </div>
        </div>
        ...
        <div class="col-md-4">
            <div class="form-group">
                ...
                <select name="section_id" class="form-control select2" disabled>
                    <option value="">{{ $studentLecture->section->name }}</option>
                </select>
            </div>
        </div>
        ...
        <div class="col-md-4">
            <div class="form-group">
                ...
                <select name="project_id" class="form-control select2" disabled>
                    <option value="">{{ $studentLecture->project->name }}</option>
                </select>
            </div>
        </div>
    ...
    <div class="form-group">
        ...
        <select name="lecture_id" class="form-control select2" disabled>
            <option value="{{ $studentLecture->lecture->name }}">{{ $studentLecture->lecture->name }} - @lang('lectures.' . $studentLecture->lecture->type)</option>
        </select>
    </div>
    ...
    <div class="form-group">
        ...
        <select name="attendance_status" id="attendance-status" class="form-control select2" required>
            ...
            @foreach (AttendanceStatusEnum::getConstants() as $attendanceStatus)
                <option value="{{ $attendanceStatus }}" {{ $attendanceStatus == $studentLecture->attendance_status ? 'selected' : '' }}>@lang('lectures.' . $attendanceStatus)</option>
            @endforeach
        </select>
    </div>

    <div id="pages-wrapper" style="display: {{ $studentLecture->attendance_status == AttendanceStatusEnum::ATTENDED ? 'block' : 'none' }}">

        <h4 class="mr-2 mt-2">@lang('pages.pages')</h4>

        <div id="pages-row-wrapper">

            @if ($studentLecture->pages_count > 0)

                @foreach ($studentLecture->pages as $index => $page)
                    ...
                    <div class="row page-row">
                        ...
                        <div class="col-md-3">
                            <div class="form-group">
                                ...
                                <input type="text" name="pages[{{ $index }}][from]" class="form-control" value="{{ old('from', $page->from) }}" required>
                            </div>
                        </div>
                        ...
                        <div class="col-md-3">
                            <div class="form-group">
                                ...
                                <input type="text" name="pages[{{ $index }}][to]" class="form-control" value="{{ old('to', $page->to) }}" required>
                            </div>
                        </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="pages[{{ $index }}][assessment]" class="form-control select2" required>
                                    ...
                                    @foreach (AssessmentEnum::getConstants() as $assessment)
                                        <option value="{{ $assessment }}" {{ $assessment == $page->assessment ? 'selected' : '' }}>@lang('pages.' . $assessment)</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
            ...
            @else
                ...
                <div class="row page-row">
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            <label>@lang('pages.from') <span class="text-danger">*</span></label>
                            <input type="text" name="pages[0][from]" class="form-control" value="{{ old('from') }}" disabled>
                        </div>
                    </div>
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            <label>@lang('pages.to') <span class="text-danger">*</span></label>
                            <input type="text" name="pages[0][to]" class="form-control" value="{{ old('to') }}" disabled>
                        </div>
                    </div>
    ...
    <div class="form-group">
        ...
        <textarea name="notes" class="form-control">{{ old('notes', $studentLecture->notes) }}</textarea>
    </div>
